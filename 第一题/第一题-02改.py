...
# 检查两个数据表的行数是否一致
if len(order) != len(supply):
    print(f"警告：订货量表和供货量表行数不一致！订货量：{len(order)}行，供货量：{len(supply)}行")
    # 取最小行数作为处理范围
    min_rows = min(len(order), len(supply))
    order = order.iloc[:min_rows]
    supply = supply.iloc[:min_rows]

# 清理供应商ID列的缺失值
order = order.dropna(subset=["供应商ID"]).reset_index(drop=True)
supply = supply.loc[supply["供应商ID"].isin(order["供应商ID"])].reset_index(drop=True)

# 确保两个表的供应商ID顺序一致
supply = supply.set_index("供应商ID").reindex(index=order["供应商ID"]).reset_index()
...
for i in range(n):
    try:
        # 确保索引在有效范围内
        if i >= len(order) or i >= len(supply):
            print(f"跳过索引{i}：超出数据范围")
            continue
        ...
        print(f"处理供应商 {supplier_ids[i] if i < len(supplier_ids) else i} 时出错: {str(e)}")
        # 为出错的供应商设置默认值
        X[i, :] = [0, 0, 0, 0]
...
# 过滤掉供应商ID为空的记录
result = result.dropna(subset=["供应商ID"])

top50 = result.head(50)
print(top50)
top50.to_excel("/mnt/top50供应商.xlsx", index=False)