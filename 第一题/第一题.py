...
for i in range(n):
    # 提取第i个供应商的订货量和供货量
    order_i = order.iloc[i, 2:].values.astype(float)
    supply_i = supply.iloc[i, 2:].values.astype(float)
...
def entropy_weight(Z):
    ...
    E = np.zeros(m)
    for j in range(m):
        p = Z[:, j] / np.sum(Z[:, j]) if np.sum(Z[:, j]) != 0 else np.zeros(n)
        p = p[p > 0]  # 排除0值
        E[j] = -np.sum(p * np.log(p)) / math.log(n) if len(p) > 0 else 0
    W = (1 - E) / np.sum(1 - E)
    return W
